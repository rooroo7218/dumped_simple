...
export function NeonText({
  ...
  return (
    <div
      className={cn("min-h-screen bg-black flex items-center justify-center font-mono", className)}
      style={
        {
    ...
    >
      <h1
        ...
        className="neon-text text-white text-8xl md:text-9xl font-light italic uppercase px-16 py-12 border-4 border-white rounded-3xl focus:outline-none select-all animate-neon-flicker"
...
export function NeonColorPicker({ currentPreset, onPresetChange, intensity, onIntensityChange }: NeonColorPickerProps) {
  ...
  return (
    <div className="fixed top-6 left-6 z-10 mb-6 font-sans">
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="bg-black/80 backdrop-blur-sm p-3 rounded-xl border border-white/20 hover:border-white/30 transition-all duration-200 mb-2"
        style={{
      ...
      <div
        className={cn(
          "bg-black/80 backdrop-blur-sm rounded-2xl border border-white/20 transition-all duration-300 overflow-hidden",
          isExpanded ? "p-4 pb-6 opacity-100 max-h-[520px] w-96" : "p-0 opacity-0 max-h-0 w-0"
        )}
      >
        <h3 className="text-white/90 text-xs font-light mb-3 tracking-wide uppercase">Gas Type</h3>
        <div className="grid grid-cols-2 gap-1.5 mb-4">
          {Object.entries(NEON_PRESETS).map(([key, preset]) => (
            <button
              ...
              className={cn(
                "px-3 py-1.5 text-[10px] rounded-lg border transition-all duration-200 text-left uppercase tracking-tighter",
                currentPreset === key
        ...
        <div className="space-y-3 pt-3 border-t border-white/10">
          <div className="flex items-center justify-between">
            <label className="text-white/90 text-[10px] uppercase font-light tracking-wide">Intensity</label>
            <span className="text-white/70 text-[10px] font-mono bg-white/5 px-2 py-1 rounded">
              {intensity.toFixed(1)}
            </span>
          </div>
          <input
            ...
            step="0.1"
            value={intensity}
            onChange={(e) => onIntensityChange(Number.parseFloat(e.target.value))}
