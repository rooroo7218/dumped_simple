        ...
        if (memory.actions && memory.actions.length > 0 && authUser?.id) {
            try {
                const actionsToSave = memory.actions.map(action => ({
                    ...
                    scheduled_time: action.scheduledTime ? new Date(action.scheduledTime).toISOString() : null,
                    x: action.x,
                    y: action.y,
                    ...
                    last_reviewed: action.lastReviewed ? new Date(action.lastReviewed).toISOString() : null,
                    context_tags: action.contextTags,
                    complete_by: action.completeBy ? new Date(action.completeBy).toISOString() : null,
                    trend: action.trend,
    ...
    async loadMemories(): Promise<MemoryItem[]> {
        ...
        const items = (mems || []).map(m => {
            const memActions = actions.filter(a => a.memory_id === m.id).map(action => ({
                ...
                scheduledTime: action.scheduled_time ? new Date(action.scheduled_time).getTime() : undefined,
                x: action.x,
                y: action.y,
                description: action.description,
                ...
                deadline: action.deadline ? new Date(action.deadline).getTime() : undefined,
                lastReviewed: action.last_reviewed ? new Date(action.last_reviewed).getTime() : undefined,
                contextTags: action.context_tags,
                completeBy: action.complete_by ? (isNaN(Date.parse(action.complete_by)) ? undefined : new Date(action.complete_by).getTime()) : undefined,
                trend: action.trend,
